...
class CustomPromptsDataset(torch.utils.data.Dataset):
    ...
    def get_batch(self, index):
        ...
        clip_length = min(video_length, 15 * 4 + 1)
        start_idx = random.randint(0, video_length - clip_length)
        batch_index = np.linspace(start_idx, start_idx + clip_length - 1, 16, dtype=int)
        pixel_values = torch.from_numpy(video_reader.get_batch(batch_index).numpy()).permute(0, 3, 1, 2).contiguous()
        pixel_values = pixel_values / 255.
        pixel_values = self.pixel_transforms(pixel_values)
        del video_reader

        return {"prompt": Annotations.clean_prompt(prompt), "pixel_values": pixel_values}
...
class PromptReader(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        return torch.utils.data.DataLoader(self.predict_dataset, batch_size=2, pin_memory=False, shuffle=True, drop_last=False)