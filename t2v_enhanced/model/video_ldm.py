...
class VideoLDM(pl.LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> Any:
        ...
        if  self.img_cond_resampler is not None:
            bsz = input_frames.shape[0]
            image_for_conditioniong = rearrange(
                input_frames_conditioning, "B F C W H -> (B F) C W H")
            image_enc = self.img_cond_encoder(image_for_conditioniong)
            img_cond = self.img_cond_resampler(image_enc, batch_size=bsz)
            image_enc_unc = self.img_cond_encoder(
                torch.zeros_like(image_for_conditioniong))
            img_cond_unc = self.img_cond_resampler(image_enc_unc, batch_size=bsz)
        else:
            img_cond = None
            img_cond_unc = None
